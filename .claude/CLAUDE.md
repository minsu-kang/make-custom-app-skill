...
The 4 installers (`install-cursor.sh`, `install-cursor.ps1`, `install-claude.sh`, `install-claude.ps1`) download the repository archive once and copy `skill/`, `rules/`, `mcp-server/` (source only), and `subagents/` wholesale. There is no per-file list — adding or removing a file needs no installer change. Still update `README.md` (Skill Files / Script Files tables, Repository Structure) and `skill/SKILL.md` (routing table, reference index) when a workflow, reference, or script is added, renamed, or removed.
