...
**Skip Breaking Changes** in five cases — evaluate before the review. Case 5 also skips **Bugs**.
...
5. **No-production-surface skip (Breaking AND Bugs)** — if the **only consumers** of the finding were never deployed to production (`private: true` / `private: null`, never made public) **or** are unused (Investigation / Dev Notes / usage = 0 users and 0 scenarios, or incomplete stub with `expect` and `parameters` both `[]`), skip both Breaking and Bugs for those consumers. The code can still be wrong; it is not a review blocker. State: `Breaking/Bug skipped — {group}/{item} has no production surface`. **Do NOT skip** when usage exists, even if labeled `(deprecated)` (e.g. `makeSoapApiCall` with 7 users / 3 scenarios). **Shared functions:** skip only the unused callers; production callers of the same function are still reviewed. **When usage is unknown, do not skip.** Combined evidence is enough: `private: null` + empty expect/parameters + Dev Notes "skipped" (IEN-14893 `generateAccountReport` / `generateCampaignReport`). See [code-review-criteria.md](../references/code-review-criteria.md) Breaking skip #5.
