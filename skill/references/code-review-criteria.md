...
- Confusing `{{environment.timezone}}` (scenario env, always available) with `flags.environmentAccess` (server-side `process.env`, requires flags) — and both with `{{internal.*}}` (Make-infrastructure data, requires the **admin-granted** `flags.exposeInternalProperties` allowlist)
