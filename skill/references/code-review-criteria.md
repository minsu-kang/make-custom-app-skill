...
- Confusing `{{environment.timezone}}` (scenario env, always available) with `flags.environmentAccess` (server-side `process.env`, requires flags) — and both with `{{internal.*}}` (Make-infrastructure data, requires the **admin-granted** `flags.exposeInternalProperties` allowlist). Don't leave a `{{internal.*}}` grant as "unconfirmed, ask admin" — verify the actual grant via `GET /sdk/apps/{slug}/{version}?cols[]=flags` (see `runtime-reference.md` § Make-Infrastructure Data → "Verifying the real grant") before deciding it's a blocker.
- **Assuming `api.endpoint` / `pagination.endpoint` names accept IML templates** — they are static component references since v1.102.3 (PR #713). A template name is used verbatim and fails the lookup; a string `endpoint.connection` is rejected outright. See `runtime-reference.md` § "Endpoint references are STATIC".
- **Flagging an Instant Trigger module (type_id 10) for "missing `attached_accounts`" without checking the paired webhook** — an Instant Trigger module has no connection configuration of its own ("There is nothing to configure in this module except the interface. The data processing is handled by a selected webhook." — [Make docs](https://developers.make.com/custom-apps-documentation/app-components/modules/instant-trigger)). Rolling a new connection out to Instant Trigger modules means wiring the **webhook's** `attached_accounts`, not the module's. The module-level field still exists at the SDK-entity level but has no runtime effect unless the module's own (optional) `api.imljson` communication actually reads `{{connection.*}}` — check that file before assuming the module-level binding matters. See `component-patterns-reference.md` § "Connection Ownership — Instant Trigger Modules Have None (the Webhook Does)".
