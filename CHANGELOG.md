...
- **New `skill/scripts/check-setup.js`** — one command replaces the SKILL.md "Hard Stop", the four setup-guide sections, and the four Pinecone setup guides. Checks skill version (via `version-guard.js`), `imt-app-runtime-path` (exists on disk), Make API credentials (Cursor `settings.json` / Claude `make-api-key`), and the optional mockup path, Jira credentials, and MCP server state (installed → built → `.env` → registered). Prints `OK` / `MISSING` / `OPTIONAL` per item plus the exact fix; exit 1 when a required item is missing; `--json` for machine output. It is the skill's first action in every conversation.
- **`skill/SKILL.md` 36 KB → 8.7 KB.** Now: first action (`check-setup.js`), workflow routing table, nine hard rules (vendor-docs verification, no invented runtime features, read-before-flag, write-script confirmation + rollback fence, review disposition gate, reviewer never writes Dev Notes / never transitions `Test` sub-tasks, no skill internals in developer-facing text, AC scope, tests ship with function edits), component and module-type tables, a "read when" reference index, and a script index. Removed: version-check prose (code enforces it), IML variable/function tables and the communication skeleton (already in the references), runtime-limit and error-type tables (moved to `runtime-reference.md` / `builtin-iml-functions.md`), the Important Notes incident dump (moved into `app-compilation-and-deployment-reference.md` § "App-level visibility"), all setup guides, and the remaining `~/.cursor/skills/...` literals.
- **`rules/` collapsed to one file.** `make-custom-app.mdc` (~600 bytes, `alwaysApply`) only tells Cursor to load the skill for Make app work. Removed: `make-app-workflow.mdc`, `make-app-todo-rules.mdc`, `make-app-todo-bugfix.mdc`, `make-app-todo-feature.mdc`, `make-app-todo-task.mdc`, `make-app-todo-review.mdc`, `make-app-todo-refinement.mdc`, `work-discipline.mdc` (22 KB that previously loaded into **every** Cursor conversation, Make-related or not). Their hard rules live in SKILL.md; the verbatim 15–18-item TODO templates are retired — each workflow ends with a 5–8 line `## Checklist` and the agent builds its own todo list. Claude Code no longer installs a `rules/` directory.
- **New `skill/workflows/lifecycle.md`** — the shared skeleton every task follows: app identification (inline → App HQ URL → IPME → ask), code sync, context load, Jira fetch (field list, subtask status filter, attachments), plan-before-edit, push with confirmation (component-path table, `commit-changes.js` options, rollback fence), close-out (Dev Notes prompt, context-file template, `upsert_app_context` / `upsert_jira_ticket`, post-commit sync). Absorbs and replaces `workflows/app-context.md` and `workflows/pinecone-sync.md`.
- **Task workflows rewritten as deltas** — `code-review.md` 43 KB → 14 KB (skip rules #1–#5 stated once, including 1.22.0's #5; runtime-reference and vendor-docs gates as procedures; output format; disposition gate with past-tense/imperative distinction; re-review), `task-refinement.md` 26 KB → 9 KB, `bug-investigation.md` 8.7 KB → 3.6 KB, `feature-request.md` 6.6 KB → 3.1 KB, `app-task.md` 5.5 KB → 2.2 KB, `create-endpoint.md` 9.4 KB → 5 KB. Workflows total 119 KB → 45 KB.
- **References** — every file gains a one-line `> Read when:` header; `code-review-criteria.md` no longer duplicates the skip-rule block (links to `code-review.md`); `builtin-iml-functions.md` carries the `errorFactory` type list; `app-compilation-and-deployment-reference.md` gains § "App-level visibility" (admin endpoint 200/403/404 semantics, `private` / `deprecated` matrix). Factual content unchanged.
- **`subagents/make-integration-engineer.md` 13 KB → 1.6 KB.** First action `Skill('make-custom-app')`, then SKILL.md is the contract. The duplicated version-check and tail-config prose and the private `~/.claude/agent-memory/` store are removed — knowledge persists only through context files and Pinecone.
- **Installers download the repository archive instead of hardcoded file lists.** All four scripts fetch `https://github.com/minsu-kang/make-custom-app-skill/archive/refs/heads/master.{tar.gz,zip}` once (or use the local clone) and copy `skill/`, `rules/`, `mcp-server/` (source only), and `subagents/` wholesale. The `SKILL_FILES` … `MCP_SERVER_FILES` arrays, `install-sync.mdc`, and the CLAUDE.md "Install Sync" section are gone — adding a file can no longer break `curl | bash` installs. On update the Cursor installer replaces `~/.cursor/rules/make-custom-app/` wholesale and removes 1.x rule files left directly under `~/.cursor/rules/`; the Claude installer drops `skills/make-custom-app/rules/`. `--update` is now fully non-interactive (the MCP key prompt is skipped when no `.env` exists). Tail-config preservation and `.env` preservation are unchanged.
- **User config moves out of `SKILL.md` into `~/.make-custom-app-skill-secrets`.** `SKILL.md` is loaded into the model's context every session, so the `make-api-key` / `jira-api-token` lines appended to its tail were being sent to the LLM on every conversation. All eight config keys (`imt-app-runtime-path`, `make-api-key`, `make-api-url`, `make-apps-mockup-path`, `jira-email`, `jira-api-token`, `jira-base-url`, `mcp-server-path`) now live in one `key: value` file in the home directory (mode 600), shared by the Cursor and Claude Code installs. Only scripts read it; SKILL.md hard rule 10 forbids the agent from opening it.
    - `scripts/lib/settings.js` — `readSkillConfig()` reads the secrets file first and falls back to the 1.x SKILL.md tail; placeholder values (`your-…`, `<…>`, `@example.com`, `/path/to/…`) and markdown blockquote samples are ignored in both. New shared `loadJiraConfig()`; `test-function.js`, `test-component.js`, `download-jira-ticket-attachment.js`, `post-review-transition.js` no longer parse SKILL.md themselves.
    - **Installers migrate automatically.** On update, every real value found in the old SKILL.md tail is moved into the secrets file (existing secrets-file keys win, no duplicates on re-run), and nothing is appended to the new SKILL.md any more. Both `.sh` and `.ps1`, both editors.
    - `check-setup.js` — every fix message now points at the secrets file; a new `legacy config in SKILL.md` line warns when config lines are still present in SKILL.md and lists them.
    - Workflows: the reviewer's Jira email for `lookupJiraAccountId` comes from `check-setup.js` output (email only, never the token); the task-refinement ADF fallback calls `settings.loadJiraConfig()` from a script instead of reading credentials.
- **Per-session load** (code review, before reading app code): ≈ 190 KB / 45k tokens → ≈ 70 KB / 17k tokens. Cursor global always-on overhead: 22 KB → 0.6 KB.
...
- **New `runtime-reference.md` — "Make-Infrastructure Data (`internal` / `flags.exposeInternalProperties`)" — the `{{internal.*}}` IML root was entirely undocumented.** Added in imt-app-runtime v1.103.0 (PRs #726 + #727), it is a third flag-gated context root alongside `environment` and `environment.system`, carrying Engine-supplied Make-infrastructure data (team identity, ISC service URNs) that `buildContext()` previously never read. Documents the admin-granted `flags.exposeInternalProperties` allowlist (enumerated array of property paths, dot paths supported), the shallow freeze and its `_.pick()` intermediate-container limitation, and that the projection is also a sandbox global reachable from custom IML functions. Verified against `lib/core/runtime.js` `buildExposedInternal()` and `test/expose-internal-properties.spec.ts`.
    - **The failure mode is silent, so it is now a review item.** A non-array grant (e.g. `true`) is degraded to no access with only a `console.error` — the execution keeps running and the app looks broken with no app-visible error, unlike `flags.environmentAccess` which throws. Recorded as `security-reference.md` — 7.3 (High) and in `code-review-criteria.md`'s "common mistakes" list next to the existing `environment` / `environmentAccess` confusion.

- **`runtime-reference.md` — "Inline Endpoint Calls" corrected — Endpoint references are STATIC, not IML-evaluated.** The section was verified 2026-07-24, before imt-app-runtime v1.102.3 (PR #713) disabled IML evaluation of `endpoint.name` and `endpoint.connection`; component references must stay statically analysable for cross-component dependency resolution. The old rules table asserted the opposite (`endpoint.name` is IML-evaluated | Like `connection`, may be a template string), so a review following the reference would have approved a template name that cannot work.
    - **Both new failure modes are documented as loud, not silent**: a template `"endpoint": "{{parameters.which}}"` misses the `app.endpoints` Map lookup and raises `Endpoint '{{parameters.which}}' not found.`; a string `endpoint.connection` raises a typed `InvalidConfigurationError`. `api.input` / `pagination.input` are untouched and remain fully IML-evaluated each round — the split is component reference vs. runtime data.
    - Dynamic Endpoint selection therefore does not exist; `code-review.md`'s directive-routing gate and `code-review-criteria.md` both now say so, so the stale claim cannot be reintroduced from either entry point.
...
- **New `skill/scripts/commit-changes.js` — the app change lifecycle is now scriptable.** The skill could read uncommitted changes (`review-changes.js`) but not act on them: committing meant the user switching to the SDK UI and reporting back "????" before `post-review-transition.js` could run. The script wraps all three lifecycle endpoints, verified against `apps-sdk-internal` `src/commands/ChangesCommands.js`: `POST /sdk/apps/{slug}/{version}/commit` (body `{ notify, message, changeIds }`, message 1—1000 chars per `Validator.commitMessage`), `POST .../rollback` (no body, all-or-nothing), `POST .../compile` (polls `compilationError`, since the compile is async in make-apps-processor). Commits **all** pending changes by default; `--change-ids=` takes a subset using the ids from `review-changes.js` output. `notify` defaults to off. Reuses the shared origin resolution, 429 backoff, and `version-guard` of the existing scripts.
    - **`--issue=KEY` chains the Jira transition.** On a successful `commit` / `compile` the script spawns `post-review-transition.js {KEY} committed`, so the forward path is one command instead of two.
    - **Pending-0 is disambiguated, not just reported.** When there is nothing to commit and `metadata.json` says `approved: false`, the script explains that a non-approved app writes SDK edits straight to the DB and points at `compile` as the forward action, instead of failing with a bare "no changes" (same trap — 5a documents for `review-changes.js`).

- **`skill/workflows/code-review.md` — "Post-Review Disposition Gate" — the user can now delegate the forward action, and tense decides who acts.** Past tense ("????" / "?????") keeps the existing path untouched: `post-review-transition.js {key} committed` ? context file ? Pinecone, no commit script. Imperative ("????" / "?????") **is** the forward disposition and satisfies the gate — run `commit-changes.js ... --issue={key}` instead, before the context/Pinecone writes so they record what actually happened, and do not transition the parent twice. Sub-task transitions and the `Test`-is-QA-territory hard rule are unchanged.

- **Rollback is fenced off in three places.** It discards every pending change on the version at once and cannot be undone, so: the script refuses to run without `--confirm`; `rules/make-app-workflow.mdc` forbids self-initiating it, proposing it as a repair for a failed commit, or passing `--confirm` on the agent's own reasoning; and both the rule and `workflows/app-context.md` state that a returned ticket ("???" / "returned") is **never** rollback authorization — returning a ticket leaves the developer's pending changes intact.

- **Supporting docs + installer sync.** New `workflows/app-context.md` — "App Commit / Rollback / Compile" (flag table, `approved` precondition, resync-after-rollback note) and a corrected "user must still commit via SDK" line; `references/app-compilation-and-deployment-reference.md` — "Commit / compile lifecycle" gains the rollback endpoint and the script cross-link; `rules/make-app-todo-review.mdc` folds the delegated commit into the existing `review_transition` item (no new todo); all 4 installer `SCRIPT_FILES` arrays, the `README.md` script table, and both `SKILL.md` script lists updated per `install-sync.mdc`.

## 1.18.2 — 2026-07-28

- **`skill/references/runtime-reference.md` — "OAuth Connection Variables" — new `token.response.scope` subsection.** The OAuth2 `token` directive gained a `response.scope` field in imt-app-runtime **v1.101.0** (2026-07-18, PR #693). Before it, the platform only stored the scopes it _requested_ — a user who granted a subset was invisible to Make (the source flags this as a known limitation). Documents the value normalization (`_normalizeScope()` accepts a string array, a space-delimited string per RFC 6749 — 3.3, a comma-delimited string, or a mix; anything else is treated as "not reported"), the precedence rule (a non-empty reported scope **overrides** the stored list rather than merging, so a revoked scope drops off on the next exchange; empty/absent falls back to merging `acceptedScope`), array-`token` behavior (last item reporting a non-empty scope wins, so a scope-less follow-up request cannot erase an earlier one), and that it is the only scope source for the Client Credentials flow. Also records that `saveScope()` is deliberately wired into `token()` only — `refresh.response.scope` is silently ignored, since a refresh is not an authorization event. Source: `accounts/app-runtime-oauth2/lib/account.js`.

- **`skill/references/component-patterns-reference.md` — "OAuth Scope Files" — companion "Recording the granted scope" subsection.** The existing section covers the three sources of _requested_ scope (`scope.imljson`, module/RPC `scope.imljson`, `additionalScopes`); this one covers what the provider reports back, with the one-line `"scope": "{{body.scope}}"` pattern and the two facts that change behavior when adding it to an existing connection (override-not-merge; `token` only, never `refresh`). Cross-links to the runtime reference for full semantics.

- **`skill/references/app-compilation-and-deployment-reference.md` — dropped verification-status hedging in favor of plain mechanism.** References describe how things work; they are not the place for confidence markers or open questions. Removed the "not yet cross-checked against the five source repos / verify before relying on internals" blockquote from — "Module visibility & hiding" and the "mechanism not source-verified" aside from — "Module metadata surface", and rewrote the module-`description` correction as a direct statement of what is reviewable from code instead of a narration of the prior review that got it wrong.

## 1.18.1 — 2026-07-27

- **`skill/references/app-compilation-and-deployment-reference.md` — 3 — new "Module metadata surface" subsection.** `GET {zone}/api/v2/admin/sdk/apps/{slug}/{version}/modules` was documented only as the place to read the `public` hard-disable flag; it actually returns the full module entity (`label`, `description`, `typeId`, `approved`/`archived`, `connection`/`altConnection`/`attachedAccounts`, `crud`, `centicreditsFormula*`). Two consequences now written down: (1) a module **description** is readable from this endpoint even though it is absent from `download-app.js` output and from `review-changes.js` diffs — this **corrects a claim recorded during the google-calendar IEN-15506 / IEN-15720 review** that a module description is "unverifiable from code, confirm in the SDK UI"; (2) on an `approved: true` app with uncommitted work, the SDK **file sections** already serve the developer's pending values while the **module entity fields** still serve the committed baseline, so a new module with pending `account_name` / `attached_accounts` change rows shows `connection: null` / `attachedAccounts: null` here while sibling committed modules show theirs populated — cross-check `review-changes.js` before reading that `null` as a missing connection binding (observed in IEN-15502; the mechanism itself is not source-verified).

- **`skill/references/runtime-reference.md` — ISC — new "AI Module Builder rollout — the canonical 9-change component set" subsection.** The ISC section documented the directive contract but nothing about the epic (IEN-15500) that keeps porting the same Polymorph module into every Make-owned app one ticket at a time (google-sheets IEN-15502, google-forms IEN-15505, google-calendar IEN-15506, slack IEN-15987, —), each producing an identical 9-change set. Adds the per-change review checklist (`app//groups` + module `api`/`expect`/`interface`/`scope`/`account_name`/`attached_accounts` + both `rpc/*/api`), promotes the two items that earlier apps only added after a production incident to required (`log.sanitize` with `request.body.auth`, since the OAuth token is deliberately in `body.auth.accessToken` — google-calendar IEN-15714; and a non-empty module `scope`, since an empty one mints a token without the permissions the backend needs — google-calendar's `403 ACCESS_TOKEN_SCOPE_INSUFFICIENT`), and lists five recurring non-findings so they stop being re-litigated per app (`api.timeout: 300000`, the vendor `baseUrl` in `base.imljson` that ISC ignores, module scopes absent from the connection's `scopes.imljson` picker catalog, the `(beta)` label suffix, and the missing `make-apps-mockup` fixture — an AC gap but never a blocker, since mocked tests intercept before `isc.initialize()`).

## 1.18.0 — 2026-07-24

- **SDK Endpoints — full skill support for the new `endpoint` app component (Endpoints RFC).** Endpoints expose one atomic third-party API call as an AI-consumable operation, compile as `IMTRPC`, and run via MCP `endpoint_execute` / platform "Run Endpoint" (not in scenarios yet). Everything verified against the live SDK admin API (google-docs v1), `imt-web-api` master (`lib/controllers/sdk/endpoints.ts`, `lib/service/sdk-endpoint.service.ts`, `lib/routers/sdk.js`), `imt-app-runtime` master (`lib/core/chainMiddleware/endpoint.ts`, `lib/api/rpc.js`, `lib/types.ts`), and `make-mcp-server-host` (`endpoints.module.ts`). Trigger: IEN-15912 (google-docs v1 SDK Endpoints) code review — `download-app.js` couldn't fetch endpoints and the skill had zero endpoint knowledge.
    - **New `skill/references/endpoints-reference.md`** — entity structure (label/description/`context.md` markdown, MCP-style `annotations`, `attachedAccounts`, consumables `centicreditsFormula*`, `public`/`approved`/`deprecated`/`archived`), full admin API surface (CRUD routes incl. `endpointInitMode: example|blank`, name pattern, JSONC sections), the camelCase-API-path vs snake_case-change-code naming mismatch, change tracking (`apps.change.table='endpoint'`; **`label`/`description` are NOT versioned** — invisible to `review-changes.js`), runtime validation caveats (Forman `required`/`default` NOT enforced on the MCP path — source-verified in `endpoint_execute` ? `executeEndpointThroughRpcWorker`, no Forman validation; "Run Endpoint" validates; fix tracked under the Executor initiative — never an app bug), skill tooling support table, and review guidance.
    - **New `skill/references/runtime-reference.md` — "Inline Endpoint Calls (`api.endpoint`)"** — a module/RPC `api.imljson` can delegate its HTTP request to a named sibling Endpoint via `"endpoint": "name"` + `"input": {...}`. Documents all guards (`endpoint`+`url` / `endpoint`+`agency` forbidden, no arrays, no nesting, not in triggers/webhook modules, max 100 calls/execution), pagination interplay (`pagination.endpoint` whole-value override, `pagination.input` + `mergeWithParent`, `pagination.endpoint.connection` unsupported), caller-context inheritance, and the endpoint result unwrap (`response.unwrap: false` / `response.iterate` opt-outs). Corrected an initial claim: `condition()` IS in the ExecuteRpc chain, but its falsy path returns `condition.default` as **data** — it cannot raise a typed error, so it cannot implement required-field validation (reconciles the IEN-16076 field test).
    - **`skill/scripts/download-app.js`** — new `=== Endpoints ===` section: saves `endpoints/{name}/{api,input_parameters,output_parameters,scope}.imljson` + `context.md` (context has no section GET path — read from the detail JSON), and `metadata.json` gains `endpoints[]` (name, label, annotations, attachedAccounts, public/approved/deprecated/archived). Local filenames follow the snake_case change codes so review diffs map 1:1 to files.
    - **Write scripts support endpoints** — `update-app.js` `endpoint/<name>/<section>` (sections PUT via camelCase path; `context` PATCHes the entity from a `.md` file), `create-component.js` `endpoint <name> <label> [connection] [description] [initMode]`, `update-component.js` `endpoint` type (`label`/`description` PATCH; `public`/`deprecated`/`archived` via dedicated POST routes), `delete-component.js` `endpoint` type. `review-changes.js` needed no change (group-agnostic).
    - **`skill/references/component-scaffold-templates.md` — "Endpoint scaffold"** — snapshot of the `model` app `endpoints/Endpoint/` template that `endpointInitMode: 'example'` clones, with recognition markers for new-component detection.
    - **`skill/workflows/code-review.md` + `skill/references/code-review-criteria.md`** — Breaking-Changes skip extended with an **endpoint skip (always)**: endpoints are not scenario-runnable, so no scenario mappings can break (a shared IML function edited for an endpoint is still evaluated under the function change); the runtime hard-gate directive index gains an `endpoint`/`input`/`pagination.endpoint`/`response.unwrap` row; never flag the MCP validation gap as a Bug.
    - **`skill/workflows/app-context.md`** — `update-app.js` component-path table gains the `endpoint/<name>/<section>` row.
    - **`skill/SKILL.md`** — App Components table gains the Endpoint row; Detailed Reference list gains `endpoints-reference.md`.
    - **All 4 installers + README synced** — `endpoints-reference.md` added to every `REFERENCE_FILES` array and the README Skill Files table (verified 17 array entries = 17 files on disk), per the install-sync rule.

## 1.17.2 — 2026-07-02

- **`mcp-server/lib/embeddings.ts` — call the OpenAI embeddings REST API via Node's built-in `fetch` (undici) instead of the `openai` SDK.** The `openai` SDK bundles `node-fetch@2`, which intermittently throws `FetchError: ... Premature close` (`ERR_STREAM_PREMATURE_CLOSE`, thrown from `Gunzip`) while decompressing gzip'd embedding responses under Node 20+/24. This broke `upsert_app_context`, `upsert_jira_ticket`, and `search_app_knowledge` (every tool that embeds text) for the make-app-context MCP server — a Pinecone sync would fail with "Premature close" even though the API key and network were fine. Rewrote `embed()`/`embedBatch()` to POST directly to `https://api.openai.com/v1/embeddings` with global `fetch`; undici handles compressed responses reliably and ignores the (unrelated) `HTTPS_PROXY` sandbox env. Same model (`text-embedding-3-small`), same env var (`OPENAI_API_KEY`), same throw-on-missing-key / throw-on-non-2xx behavior. The `openai` dependency is now unused by the embeddings path (left in `package.json` for now; safe to drop later). All 50 mcp-server tests still pass (they mock `lib/embeddings.js`, so the internal transport change is transparent). Trigger: an IEN-11341 (google-photos v2) re-review could not sync its context/tickets to Pinecone — every `upsert_*` call returned `Invalid response body while trying to fetch https://api.openai.com/v1/embeddings: Premature close`, while a direct `curl`/undici request with the same key succeeded, isolating the fault to the SDK's node-fetch gzip handling.

- **`skill/references/parameters-reference.md` — Validation — documented that `validate` (incl. `pattern`) is enforced at back-end execution on resolved (mapped) values, and that `validate.pattern` is case-sensitive.** Two facts learned from the `@integromat/forman` source during the make-ai-web-scraper v1 review (IEN-15781):
    - **`validate` is NOT UI-only.** The type validators live in `@integromat/forman` (`lib/types/{text,url,number,—}.ts`, each headed _"This file is used by both front-end and back-end."_; central entry `lib/types.ts` ? `validate()`). At scenario execution, Make core resolves every mapping and then casts + validates each resolved parameter through the same forman `validate()`, which runs `min`/`max`/`pattern`/etc. and throws `ValidationError`. So a **mapped** value (`{{1.url}}`) that resolves to a disallowed string is rejected at runtime exactly like a typed literal. `imt-app-runtime` (the HTTP-exec layer) has no param-spec validation, but the layer above it (Make core) does — do **not** conclude "validation is UI-only" from its absence.
    - **`validate.pattern` is case-sensitive.** Make compiles it as `new RegExp(pattern, 'u')` — the `u` flag only, no `i` (`imt-forman/lib/types/text.ts`). A lowercase-only pattern lets mixed-case values through (`https://Facebook.com` bypasses a `facebook`-only deny-list), and `^https://` rejects an uppercase `HTTPS://` scheme. For case-insensitive matching, bake case into the pattern (char classes) or enforce in a custom IML function (`toLowerCase()`), which is case-robust for typed + mapped values.
- **`skill/references/code-review-criteria.md` — UX Guideline Compliance — added a `### validate.pattern Behavior` reviewer note.** Prevents the exact false-positive made in the IEN-15781 review: **do NOT flag** "a mapped value bypasses `validate.pattern` at runtime" (it does not — the pattern is runtime-enforced on resolved values); **DO flag** a lowercase-only pattern used for a security/compliance rule as a Bug (mixed-case bypass, verified empirically).

## 1.17.0 — 2026-07-01

- **Resolve app slug + version from a Jira ticket's App HQ URL (`customfield_10268`) when the user sends only a Jira link.** Previously, a message containing only a Jira link (no inline app name/version) forced the agent to ask for the app or guess it. The ticket already carries an **App HQ URL** field (`customfield_10268`, e.g. `https://eu1.make.com/admin/apps/reddit/1`), so the agent can now extract slug (`reddit`), version (`1`), and zone (`eu1.make.com`) directly. This is more precise than IPME (which only yields the latest major version), so it is preferred whenever a Jira ticket is the entry point; fallback order when the field is empty/absent is inline app name ? IPME ? ask the user.
    - **`skill/workflows/app-context.md`** — added **Condition 3** (Jira-link-only entry) and a new **"App Slug + Version from Jira App HQ URL"** section documenting both URL shapes (`/apps/{slug}/{version}` and `/admin/apps/{slug}/{version}`), the extraction rule (segments after `/apps/`, ignore optional `/admin`, strip trailing slash/query), and the IPME-fallback order. Reworded the "either of the two conditions" phrasing to "any of the conditions".
    - **`rules/make-app-todo-rules.mdc`** — `common_context_load` now spells out the 4-step slug+version **resolution order** (inline ? App HQ URL ? IPME ? ask); `common_jira_fetch` field list gains `customfield_10268` with a note on its purpose.
    - **`skill/workflows/code-review.md` — 3** and **`skill/workflows/task-refinement.md`** — added `customfield_10268` to the `getJiraIssue` field lists and documented it as **App HQ URL ? app slug + version**.
    - **`rules/work-discipline.mdc`** — added `customfield_10268` to the mandatory `getJiraIssue` fields reminder. Field label verified live against IEN-15852 (`names.customfield_10268 = "App HQ URL"`).

## 1.16.1 — 2026-06-24

- **`skill/references/parameters-reference.md` — expanded with parameter-spec features cross-referenced from imt-forman (the Make parameter form renderer) and verified against real custom apps.** The reference previously documented only a thin slice of the parameter spec surface. Audited imt-forman's `docs/` (common-properties, validation, conditional-nesting, pick/map-mode, type-aliases, rpc-providers, and per-input pages) and cross-checked every candidate property against downloaded app code before documenting. New sections added: (1) **Common Spec Properties** — `placeholder`, `mode` (pick/map/edit), `omit`, `disabled`, `readonly`, `dynamic`, `semantic` (`file:name`/`file:data`), `mappable` boolean form, plus a note listing web-renderer-only props (`visible`, `isolated`, `erasable`, —) that are NOT part of the custom-app contract; (2) **Validation** — type-specific `validate` keys (`min`/`max`/`pattern` for text, numeric bounds, date `{past:false}`, array/multi-select `minItems`/`maxItems`/`unique` incl. `unique.by`), and `validate: false`; (3) **Select Properties** — `multiple`/`grouped`/`dropdown`/`sort`/`dynamic`/`omit`/`mode` + option-level `default`/`description`/`pill`/`icon`, image options, and the feature-flag-gated `pagination`/`searchable`; (4) **Array Properties** — `labels:{add,item}`, `validate.unique`, text-style `rpc` add-button, `mappable:false`, `sequence`; (5) **Text Type Extras** — `multiline`/`rows`/`prefix`/`postfix`/`placeholder` + the `rpc` search button; (6) **Boolean Type & Conditional Nesting** — boolean/`text` `nested`, `reversedNested`, and the object `nested:{true,false}` two-branch form; (7) **Date/Time Type Extras** — `time:false`, `validate:{past:false}`; (8) **Type Aliases & Boolean Rendering** — `checkbox`?`boolean`, `uuid`/`path`?`text`, and 2-state vs 3-state boolean rendering. Properties found only in imt-forman docs (not in any downloaded app — `reversedNested`, object `nested`, `dropdown`, `rows`, `sequence`, image options, `pill`/`icon`) are marked "verify before use"; platform-only input types (`account`/`agent`/`datastore`/`scenario`/`udt`/`mcplist`/`keychain`/`device`/`hook`) and the forman-internal `isContext` mechanism (custom apps use automatic nested inheritance + query params instead) were deliberately excluded.
...
- **Code-review subtask status filter corrected: `Complete` ? `Test`.** The IEN board no longer has a `Complete` status. The actual workflow is: QA creates a subtask and returns it to the developer ? the developer fixes it and sets the subtask to **`Test`** ? once all subtasks are done the developer sets the **parent** to **`Commit`** and requests review ? the reviewer, on a `committed` disposition, moves the **parent** to **`In Testing`** ? QA then tests each subtask directly and transitions it **`Test` ? `Done`** (verified) or returns it to the developer. Updated `skill/workflows/code-review.md` — 3 (status filter `Done = skip / Test = review / Other = context`, added a "Subtask status lifecycle" note, fixed the assignment-scope example list), the — "Post-Review Disposition Gate" Hard Rule (`Sub-task "Test" status is QA territory`, `Test ? Done` belongs to QA), `rules/make-app-todo-rules.mdc` `common_jira_fetch` filter, and the `rules/make-app-todo-review.mdc` reviewer-assignment example list. No script change — `post-review-transition.js` never hardcoded `Complete` and already aborts on a `Test` subtask (QA territory). Trigger: IEN-15122 Round #6 review where the reviewer correctly left `Test`-status subtask IEN-15442 untouched and moved only the parent to `In Testing`.

## 1.14.7 — 2026-05-29

- **`skill/references/component-patterns-reference.md` — new Connection-Pattern sub-section "OAuth Scope Files — `scope.imljson` vs `scopes.imljson`".** Documents the opposite shapes of the two near-identically-named connection files: `scope.imljson` is an **array** of default active scope strings (`["<scope-url>", ...]`), `scopes.imljson` is an **object** catalog keyed by scope ? human-readable description (`{ "<scope>": "<desc>" }`) that populates the `additionalScopes` picker UI. Explains how the active scope set is composed at OAuth `authorize` time (connection `scope.imljson` ? triggering module/RPC `scope.imljson` ? user-selected `additionalScopes`), with real examples from `google-sheets-v2` / `productboard-v1` (populated) and the many apps that ship an empty `{}` (`google-ads-conversions`, `whatsapp-business-cloud`, `cloudconvert`, `github`, `google-merchant`, etc.). Includes a false-positive guard: empty `{}` is valid, the real bug is the inverted shape (`scopes.imljson` as `[]` or `scope.imljson` as `{}`).
- **`skill/references/code-review-criteria.md` — "Out of Scope (Do NOT flag)" — new entry "`scopes.imljson` Empty Object `{}`".** Stops reviewers from flagging an empty `scopes.imljson` `{}` as an improvement or suggesting `{}` ? `[]` (which inverts the schema). Table contrasts the correct vs wrong shape for both `scope.imljson` and `scopes.imljson`, and cross-links the full mechanism in `component-patterns-reference.md`. Trigger case: google-merchant v1 review (IEN-12847) where `scopes.imljson: {}` was incorrectly flagged as needing to become `[]`.

## 1.14.6 — 2026-05-28

- **`rules/work-discipline.mdc` — 7 — new sub-section "External API Reference Verification (mandatory before any external-API claim)".** Codifies that `runtime-reference.md` and the in-skill references describe Make's runtime, NOT the third-party vendor's contract — every claim about the external API surface (endpoint params, body fields, response shape, "module is missing parameter X") must be verified against the vendor's official docs before being asserted in code or in a code review. Six trigger phrases enumerated (qs/body/header/response/missing-param/symmetric-endpoints), five verification rules (no claims from memory, `WebFetch`/`WebSearch` against vendor docs first, "I couldn't verify {X}" instead of guessing, withdraw immediately on contradiction, record verified facts to `${CONTEXTS_DIR}/{slug}-v{version}.md` — Caveats with the source URL). Trigger case: IEN-15238 (Etsy v2) where two unverified claims (`includes` on `findAllListingsActive`, `readiness_state_on_property` mis-graded as "Improvement") shipped in a single review pass — first wrong, second a real bug under-flagged.
- **`skill/workflows/code-review.md` — 7 — new "Hard Gate — External API Reference Verification" peer-section to the existing "Hard Gate — Runtime Reference Read".** Six-step procedure (identify exact endpoint ? fetch vendor docs via `WebFetch`/`WebSearch` ? quote source in review output ? "I couldn't verify {X}" when fetch fails ? existing app code is not proof, symmetric-looking endpoints often diverge ? withdraw on contradiction). Documents two real false-positives prevented (Etsy `/listings/active` does not accept `includes`; Etsy `updateListingInventory` accepts `readiness_state_on_property` as first-class body field — was graded too low). Review Categories list updated with new "External API Verification" bullet pointing to the gate.
- **`skill/workflows/code-review.md` — "Developer Message" — new "Audience Rule — No Skill-Repo Commands in Developer Messages" sub-section.** The "To Developer" message is read by developers who do not use this skill — they work in the Make Internal App (VS Code extension) and the SDK web UI. Skill-internal vocabulary in their copy-paste is noise at best, confusing at worst. Forbidden in any developer-facing message: skill scripts (`update-component.js`, `update-app.js`, `test-component.js`, `post-review-transition.js`, etc.), skill paths (`~/.cursor/skills/...`, `${SKILL_ROOT}`, `${CONTEXTS_DIR}`, `~/.cursor/make-app-contexts/...`), skill-internal references (`Pinecone`, `upsert_app_context`, `make-app-context` MCP, `${slug}-v${version}.md`), skill flags (`public=true`, `--debug`, `--format=json`). Translation rule: speak in the developer's language (IMLJSON / SDK terms with in-app relative paths like `modules/SearchTiles/expect.imljson`). Applies to the To Developer block, any quoted snippet a user might copy to Jira, and Jira comments / Developer Notes authored on the developer's behalf. Does NOT apply to internal review analysis above the To Developer line (user is the audience there) or skill-side post-review actions (silent local execution).
...
- **`skill/workflows/task-refinement.md` — "Output Format" — Investigation subtask body delivery flipped to ADF-default with mandatory `expand`-wrapping for every code block.** Two new hard rules added to the Output Format section (raising the count from 2 to 4): (3) every code block in the subtask body must be wrapped in an ADF `expand` element with the file path as `expand.attrs.title` (one `expand` per file — `installSpec.imljson`, `install.imljson`, `common.imljson`, `metadata.json`, `base.imljson`, `groups.imljson`, every `modules/{name}/*.imljson`, every `functions/{name}/code.js`, plus reference snippets like `External API response shape`); (4) the subtask description body is English-only regardless of chat language — summary, prose, table cells, `expand` titles, Open Questions. Prose / tables / headings / lists stay at the top level; only code blocks go inside `expand`. — "Subtask body delivery" section now documents the minimum `expand` JSON shape and recommends programmatic ADF construction (Python `urllib.request` against `POST /rest/api/3/issue` using the `jira-email` + `jira-api-token` from `SKILL.md` tail config) over hand-written ADF when the Atlassian MCP `createJiraIssue` tool is unavailable or returns auth errors. Markdown fallback retained only for the rare zero-code-block refinement (UX-only, metadata-only). The template at — "Template" is now explicitly labeled a structural reference, not the wire format. Codified after IEN-15430 (Investigation sub-task for IEN-14735 Make AI Web Scraper) shipped with 13 `expand`-wrapped code blocks per user request — pattern is now the default for every future — P refinement.

## 1.14.3 — 2026-05-27

- **`skill/references/component-patterns-reference.md` Connection Flow rewrite.** Clarifies that `parameters.*` and `connection.*` are aliases over the same `account.data` store — user inputs in connection `parameters.imljson` are accessible as `{{connection.fieldName}}` from every context (connection api, modules, RPCs, webhooks) **without** any explicit `response.data` save step. Sources: `imt-app-runtime/accounts/app-runtime-oauth2/lib/account.js` lines 86—87 (and OAuth1 / Basic equivalents) initialize the apiContext with `parameters: () => this.data, connection: () => this.data`. The `response.data` row of the directives table now correctly scopes its purpose to (a) persisting API-derived values (tokens, expiry, account ids) and (b) transforming a user input before storage. Closes a recurring false-positive where reviewers flagged "missing `token.response.data.fieldName` save" as a bug when the field was already in `parameters.imljson`.
...
- Add shared skill/scripts/lib/skill-root.js utility (no \_\_dirname)
...
- `hooks/make-app-auto-actions-check.js` — fix —8 Developer Notes false-positive that re-prompted across sessions for tickets the user had already declined. The previous `detectDevNotesDecline` only matched explicit "no + notes-keyword" phrases (`"don't write developer notes"`, `"?——— ——?——"`), so context-implicit Korean replies like `"??"` / `"?? ———?——?————"` (right after the agent's "Developer Notes? ———??—————?" prompt) were never recognized as declines and the hook kept asking forever. Detection now walks messages in conversation order and accepts a second form: a short standalone-negation user reply (`??`, `?`, `?——`, `?`, `?———?`, `no`, `nope`, `nah`, `skip`, `pass`, `decline`, `not now`, `no thanks`, —) immediately following an assistant turn that contained the dev-notes prompt. The original explicit-form regex is preserved for cases where the user volunteers a decline without being prompted.
...
- `hooks/make-app-auto-actions-check.js` — fix —8 (Developer Notes) false positives:
    ...
    2. Broaden the user-decline detector to Korean (`————— —— ?——?`, `?? ——`, `—— ——`, `?——— ?`, `———?`, `———?`, etc. adjacent to `dev note` / `developer notes` / `——————— ?———` / `?———`) and scan the **entire** user-message history instead of only the last message — once the user has declined, the rule says "skip, do not ask again" and the decision must persist across turns.
    3. Broaden the "already prompted" detector to match Korean wording (`Developer Notes? ———??—————`, `——————— ?——— —` etc.) and to inspect `AskQuestion` tool inputs in addition to assistant prose.
